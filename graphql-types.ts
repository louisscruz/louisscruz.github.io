...
export type GetAllBlogPostsQuery = { allMdx: (
    Pick<MdxConnection, 'totalCount'>
    & { edges: Array<{ node: (
        Pick<Mdx, 'id' | 'slug' | 'excerpt' | 'timeToRead'>
        & { frontmatter?: Maybe<Pick<MdxFrontmatter, 'title' | 'date'>>, wordCount?: Maybe<Pick<MdxWordCount, 'paragraphs' | 'sentences' | 'words'>> }
      ) }> }
  ) };
...
export type BlogPostQueryQuery = { mdx?: Maybe<(
    Pick<Mdx, 'body' | 'id' | 'slug' | 'timeToRead'>
    & { frontmatter?: Maybe<Pick<MdxFrontmatter, 'title' | 'date'>>, wordCount?: Maybe<Pick<MdxWordCount, 'words'>> }
  )> };
